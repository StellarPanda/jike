...
- 请求参数：`databaseId`、`queryText`、`exportFormat`（`csv` 或 `json`）
- 导出接口复用只读校验、默认 `LIMIT 1000` 和查询历史记录
- 前端在查询结果区提供 CSV/JSON 一键下载
...
- 支持将查询结果导出为 CSV 和 JSON 文件
- 导出前复用查询执行链路，确保只允许安全的只读 SQL
- 用户执行查询后，可以在结果区点击对应格式按钮下载
- 提供 `scripts/export-query.sh` 命令和 Claude Code 自定义命令说明，支持通过命令触发查询并导出

### 8.2 自动化流程

1. 接收数据库 ID、SQL 和导出格式。
2. 解析并校验 SQL，只允许单条只读查询。
3. 执行查询并记录查询历史。
4. 将列名和结果行格式化为 CSV 或 JSON。
5. 返回带文件名的下载响应，或由命令保存到本地文件。

### 8.3 验收标准

- CSV 文件包含表头和全部查询结果。
- JSON 文件包含执行 SQL、行数、列信息和结果行。
- 前端查询结果为空时，导出按钮不可用。
- 导出失败时，页面和命令都能显示明确错误。
- `FEATURE_EXPORT.md` 记录设计思路、接口、自动化流程和验证方式。
...
- Phase 1：已完成，前后端骨架、SQLite 和健康检查可运行。
- Phase 2：已完成，PostgreSQL 连接、元数据、手写 SQL、结果和历史已打通。
- Phase 3：已完成，Schema 注入、自然语言生成 SQL 和生成结果执行已接入。
- Phase 4：已完成，文档、错误态、连接删除和交付检查已收口。
- 第三周数据导出作业：已完成，CSV/JSON 导出、前端一键下载、自动化命令和作业说明已接入。
